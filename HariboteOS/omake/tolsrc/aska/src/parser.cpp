...
//		}
	  ...
	  case TK_LOOP:
		LoopStatement();
	K_DWORD: case TK_CHAR: case TK_SHORT: case TK_LONG:
	  case TK_INT: case TK_STATIC: case TK_SIGNED: case TK_UNSIGNED:
		DefineVariable();
		break;
	  case TK_STRUCT:
...
void	Parser::WhileStatement(void){
	...
	generator.Jump(cmptype, labelcount+1);
	StatementSequence();
	generator.Jump(C_JMP, labelcount);			// while文のはじめへジ"");
			return;
		}
		generator.Seek(linedata1);
		StatementSequence();
	...
	generator.Jump(C_JMP, labelcount);			// loop譁\";\"");
		return;
	}
	nest = LoopLabelPoint - nest;
	if(nest < 0){
	...
	generator.Jump(cmptype, LoopLabel[nest] - 1);		// -1�����loop�̐擪�ɃW�����v
}
